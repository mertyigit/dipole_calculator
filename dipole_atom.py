...
def dipole_jw(coordinate, charge, number_of_molecules, first_frame, last_frame, step, a, b, c):
    ...
    try:
        ...
        dipole_w = [0 for x in range(number_of_atoms)]
        ...
        for k in range(first_frame, last_frame, step):
            for j in range(0, number_of_atoms):
                dipole[0] =  float(coordinate[k][j][1])*float(charge[k][j][1])*1.6e-29
                dipole[1] =  float(coordinate[k][j][2])*float(charge[k][j][1])*1.6e-29
                dipole[2] =  float(coordinate[k][j][3])*float(charge[k][j][1])*1.6e-29
                atom = atom + 1
